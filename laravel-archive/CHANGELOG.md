...
## 2026-09-14 — Penyelarasan UI Next.js 1:1 dengan Blade & Skrip Seeder Supabase

**Dikerjakan:** Antigravity

### Berubah

- Merekonstruksi `web/src/components/layout/Navbar.tsx`, `web/src/components/layout/Footer.tsx`, dan `web/src/app/page.tsx` agar memiliki struktur wrapper `div`, kelas utilitas token semantik (`.section-heading`, `.empty-state`, `.surface-card`, `.site-hero`, `.sambutan-feature`, dll.), SVG icon, dan alur interaktivitas yang selaras 1:1 dengan `navbar.blade.php`, `footer.blade.php`, dan `beranda.blade.php`.
- Menyesuaikan struktur navigasi Next.js: menu "Kesiswaan" dihapus dan digabungkan di bawah "Akademik" (Kurikulum, Guru & Tenaga Kependidikan, Ekstrakurikuler, Galeri, Prestasi, Organisasi Siswa, Tata Tertib) mengikuti arsitektur Laravel.
- Membuat skrip `web/seed.ts` dan `web/supabase/seed.sql` untuk mengunggah aset foto dari `database/seeders/assets/` ke Supabase Storage (bucket `images`) dan menyemai seluruh data profil resmi (`pengaturan_situs`, `guru`, `konten_halaman`, `ekstrakurikuler`, `album`, `berita`) sesuai `docs/KONTEN-SEKOLAH.md`.

### Diperbaiki

- Memperbaiki variabel font Newsreader di `web/src/app/layout.tsx` dari `--font-serif` menjadi `--font-heading` agar konsisten dengan deklarasi `@theme` di `globals.css`.
- Memperbaiki penanganan DNS Node.js pada lingkungan lokal (`dns.setDefaultResultOrder('ipv4first')`) yang sebelumnya menyebabkan `ECONNRESET` saat mengakses domain Supabase via IPv6 NAT64.
- Memperbaiki deklarasi tipe TypeScript augmentasi Inertia v2 pada `resources/js/types.d.ts` dan pemanggilan `usePage<SharedProps>()` pada `resources/js/Layouts/Layout.tsx` yang sebelumnya memicu galat kompilasi `tsc`.

### Diputuskan

- Operasi seeder database/storage Supabase dari CLI membutuhkan `SUPABASE_SERVICE_ROLE_KEY` di `web/.env.local` atau eksekusi SQL melalui Supabase Dashboard SQL Editor (`web/supabase/seed.sql`), karena kebijakan RLS membatasi hak akses role `anon` untuk mutasi tabel dan upload bucket.

### Verifikasi

- PHPUnit: 122 pengujian lolos (803 assertions, 0 gagal).
- Pint: Lolos tanpa perubahan format.
- PHPStan: Lolos level 5 (0 error).
- TypeScript: `tsc --noEmit` lolos (0 error).
- Vite: `vite build` berhasil (2494 modul terkompilasi, 1.28 detik).
- Next.js: `pnpm build` berhasil (12/12 halaman statis ter-generate).
