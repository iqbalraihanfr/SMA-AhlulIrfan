<x-layout.situs judul="Profil Sekolah">
    ...
    <div class="section-shell space-y-20 py-14 sm:py-20">
        ...
        @if ($sejarah)
            <section id="sejarah">
                <x-ui.section-heading kicker="Perjalanan sekolah" :judul="$sejarah->judul" />
                <div class="mt-8"><x-ui.prosa :html="$sejarah->isi" /></div>
            </section>
        @endif

        @if ($visiMisi)
            <section id="visi-misi">
                <x-ui.section-heading kicker="Arah pendidikan" :judul="$visiMisi->judul" />
                <div class="mt-8"><x-ui.prosa :html="$visiMisi->isi" /></div>
            </section>
        @endif

        @if ($sambutan)
            <section id="sambutan">
                <x-ui.section-heading kicker="Kepala sekolah" :judul="$sambutan->judul" />
                <div class="mt-8"><x-ui.prosa :html="$sambutan->isi" /></div>
            </section>
        @endif
